...
// injectHandlers returns a ServeMux with injected app routes.
func injectHandlers(config *Config) *http.ServeMux {
	mux := config.ServeMux
	mux.HandleFunc("/", welcomeHandler)
	mux.Handle("/manager", requireLogin(http.HandlerFunc(managerHandler)))
...
func issueSession() http.Handler {
	fn := func(w http.ResponseWriter, req *http.Request) {
		...
		session.Save(w)
		http.Redirect(w, req, "/manager", http.StatusFound)
	}
...
// managerHandler shows protected user content.
...
// InjectHttp creates and starts a Server listening.
func InjectAuth(serveMux *http.ServeMux) *http.ServeMux {
	// read credentials from environment variables if available
	config := &Config{
		FacebookClientID:     os.Getenv("LIVE_BADGE_ID"),
		FacebookClientSecret: os.Getenv("LIVE_BADGE_SECRET"),
		ServeMux:             serveMux,
	}
